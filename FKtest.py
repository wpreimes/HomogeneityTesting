...
def FKtest(DF_X,mode='median',alpha=0,createlog=False):
    ...
    if mode=='median':
        for i in range(K):
            ...
            DF_X.ix[DF_X['group']==i,'groupme_diff']=np.abs(subset['data']-groupmed) #difference data-groupmedians
    ...
    if mode=='mean':
        for i in range(K):
            ...
            DF_X.ix[DF_X['group']==i,'groupme_diff']=np.abs(subset['data']-groupmean) #difference data-groupmeans
